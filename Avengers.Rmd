...
* Inappropriate Alghoritm (?)
* Inappropriate fit of data for ELO
* "Implicit Strenghts" (ex. Giant Chitauri)
```{r}
scores %>% filter(str_detect(charater, regex("Giant")))
```

* "Cinematic Distorsion" (ex. US Army Soldiers)
```{r}
scores %>% filter(str_detect(charater, regex("(Soldier)|(Agent)|civilian")))
```
