...
fights_tidy <- raw_avengers_tibble_3 %>%
  ...
  mutate(
    winner_n = strtoi(winner_n), 
    loser_n = strtoi(loser_n)/divide_loser_n_by
    ) %>% 
...
### Specific Charater and random dudes
```{r}

# isolate specific charaters
specific_charaters <- 
  
  raw_avengers_tibble_2 %>%
  filter(!str_detect(winner, "-")) %>%
  select(winner)%>%
  rename(charater=winner) %>%
  
  bind_rows(
    raw_avengers_tibble %>%
    filter(!str_detect(loser, "-")) %>%
    select(loser)%>%
    rename(charater=loser)
  )%>%
  distinct()

# isolate random dudes
random_dudes <- all_charaters %>% 
  anti_join(specific_charaters, by="charater")

# create charaters table
all_charaters_2 <- all_charaters %>%
  
  # add "specific charater" label
  left_join(
    specific_charaters %>%
      mutate(charater_type="specific charater")
    , by="charater"
  ) %>%
  
  # add "specific charater" label
  mutate(
    charater_type=ifelse(is.na(charater_type), "random dude", charater_type)
    )
...
all_charaters_statistics <- fights_tidy %>% 
  ...
  # order columns
  select(charater, charater_type, n_fights, n_win, n_lose)
...
fights_elo_format <- fights_elo_format %>% 
  mutate(
    score=0.5 + score/2
  )
